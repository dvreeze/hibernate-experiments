...
- Don't pretend to deal *only with Java objects* (and not SQL), expecting Hibernate to take care of all "database work"
  - Note that well-known Hibernate experts know databases and SQL well
- Don't ignore *performance*, in particular the (amount of) SQL generated by Hibernate; log and even try to predict the generated SQL
...
- Language
- Film
- Film_Actor and Film_Category (with compound primary keys)
...
public class LanguageEntity { // Not serializable
    ...
    @Basic(optional = false)
    @Column(columnDefinition = "bpchar") // PostgreSQL-specific type
    private String name;
...
public interface FilmService {
    ...
    // Returns Guava ImmutableList of mutable entities (so not very useful here)
...
We fix these problems (`LazyInitializationException` and N + 1 select issue) by using *per-query fetching*. This should *not be decided (globally) at the entity level*!
...
Yet EAGER is like saying: whenever you query in SQL for data from table Film, you *always* have to join with table Language to fetch its data too.
...
Experience in previous project: we did not know about "per-query fetching" and ended up with strange reflection-based code to tell Hibernate what data we wanted to fetch.
-->
...
public final class AlternativeFilmService implements FilmService {
    ...
    public ImmutableList<Film> findFilmsByActorId(long actorId) {
        return emf.callInTransaction(StatelessSession.class, statelessSession -> {
            ...
            JpaCriteriaQuery<String> cq = cb.createQuery(QL_STRING, String.class); // Nice!
            ...
            return statelessSession.createQuery(cq).getResultList().stream() // not getResultStream()
...
private static EntityManagerFactory createEntityManagerFactory() {
    ...
    return new PersistenceConfiguration(persistenceUnitName)
            ...
            .defaultToOneFetchType(FetchType.LAZY) // since Jakarta Persistence 4.0 (do use it)!
...
- We do not have to use `Session`/`EntityManager`, but can use the more predictable (but quite different) `StatelessSession`/`EntityAgent` instead
